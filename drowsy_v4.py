...
def sms_fun():
    ...
    payload = "sender_id=TXTIND&message= Driver Sleepy &route=v3&numbers={},{}".format(number1)
    headers = {
...
mobile1 = int(input("Enter that mobile1 number u need to send alert Message\n---->>")) 
mobile2 = int(input("Enter that mobile2 number u need to send alert Message\n-----$$"))
...
while True:
     ...
     for face in faces:
          ...
          elif ((total_yawns > 4 and total_yawns <7)  or (total_blinks > 7 and total_blinks <=9)) and warning <2:
               alert()
               warning += 1
               print(warning)
               
          elif total_yawns > 7 or total_blinks >9 :
               sms_fun()
               total_yawns,warning,total_blinks =0,0,0
